...
def attr_id(attr):
    ...
    if attrib == property:
        ...
        return "meth", '.'.join([module.__module__,attrib.__qualname__])
...
class MdBase(ABC):
    ...
    @property
    def pad_space(self):
        return '\n' if self.endline or self.begline else ' '
...
class MdIlink(MdLink, kind='ilink'):
    def __init__(self, match):
        if self.ref in self.dests['*docs*']:
            ...
            self.type = 'ref'
    def make(self):
        return f':{self.type}:`{self.name} <{self.ref}>`'
    @property
    def begline(self):
        return 0
    @property
...
class MdHeading(MdHighLevel):
    ...
    def can_keep(atom):
        if isinstance(atom, MdAtomic) and atom.begline == 0 and atom.endline == 0:
            ...
            return (-1, atom)
    def __new__(cls, match, *args):
        # just to catch empty lines
        if match.group(2) is None and len(args) == 0:
            return MdText('\n'+match.group()+'\n')
        else:
    ...
    def __init__(self, match, *args):
        ...
        if text is not None:
            ...
            elements = list()
        elements += [arg if isinstance(arg, MdAtomic) else arg.text for arg in args]
        self.elements = cell_strings(elements)
        self.level = len(level) - 1
    def make(self):
        heading = render_sub(self.elements)
...
note_strip = lambda text: note_strip_regex.sub(note_strip_sub, text)
...
class MdNote(MdHighLevel):
    ...
    def can_keep(atom):
        if isinstance(atom, (MdAtomic, MdUtext)):
            ...
            return (1, atom)
    def __init__(self, match, *args):
        start_text = note_strip(match.group())
        label = note_label_regex.match(start_text)
        if label:
            self.label = label.group(2).lower()
            start_text = label.group(3).split('\n', 1)
            if len(start_text) == 2:
                start_text = start_text[0].strip() + '\n' + dedent(start_text[1])
            else:
                start_text = start_text[0].strip()
        else:
            self.label = None
            start_text = dedent(start_text)
        arg_strip = [MdUtext(note_strip(arg.text)) if isinstance(arg, MdUtext) else arg for arg in args]
        self.elements = procHighlevel([MdUtext(start_text.strip('\n ')), ] + arg_strip)
        
    def make(self):
        if self.label is None:
            return indent(render_RST(self.elements))
        else:
            return trail_whitespace_regex.sub(r'\1', f'.. {self.label}::\n\n' + indent(render_RST(self.elements)))
    @property
    def begline(self):
        return 2
    @property
...
wsp_regex = re.compile(r'[\n ]')
ech = lambda text: text
echm = lambda match: match.group()
...
def cell_min_span(strings, min_span):
    width = max(chain.from_iterable((len(line) for line in string.split('\n')) for string in strings))
    rows = sum(string.count('\n') + 1 for string in strings)
    if width < min_span:
        return min_span if width > rows else rows
    else:
        return width
    


def whitespace_greater(a, b):
    anl, bnl = trail_newline(a), trail_newline(b)
    if anl == bnl:
        asp, bsp = trail_whitespace(a), trail_whitespace(b)
        return a if asp > bsp else b
    else:
        return a if anl > bnl else b

def cell_iter(components):
    comp_iter = iter(components)
    for comp in comp_iter:
        if isinstance(comp, str):
            yield from (r for r in regex_split_iter(wsp_regex, comp, ech, echm) if r)
        else:
            yield comp

            
def is_joinpad(comp_next):
    if isinstance(comp_next, str):
        return comp_next not in (' ', '\n')
    else:
        return comp_next.pad_space != '\n'

def cell_strings(components):
    comp_prev = ''
    endline = 0
    citer = cell_iter(components)
    for comp in citer:
        if isinstance(comp, str):
            if isinstance(comp_prev, str) and whitespace_regex.match(comp_prev) and whitespace_regex.match(comp):
                comp_prev = whitespace_greater(comp_prev, comp)
            else:
                if comp_prev:
                    if isinstance(comp_prev, MdAtomic):
                        yield comp_prev.make() + '\ '
                    else:
                        yield comp_prev
                comp_prev = comp
        else:
            ...
            if pad in (' ', '\n'):
                if pad == '\n':
                    begline, endline = comp.begline, trail_newline(comp_prev)
                    if len(comp_prev) != comp_prev.count(' ') and comp_prev:
                        yield comp_prev
                    if endline < comp.begline and comp_prev:
                        yield '\n'*(begline - endline)
                    yield comp.make()
                    comp_prev = '\n'* comp.endline
                else:
                    if isinstance(comp_prev, MdAtomic) or not trail_whitespace(comp_prev):
                        txt =  comp_prev if isinstance(comp_prev, str) else comp_prev.make()
                        yield txt + '\ ' + comp.make()
                    elif trail_whitespace(comp_prev):
                        yield comp_prev
                        yield comp.make()
                    else:
                        yield comp_prev
                ...
                if trail_whitespace(comp_prev):
                    yield comp_prev
                    text = comp.make()
                else:
                    text = comp_prev + pad + comp.make()
                try:
                    comp_next = next(citer)
                except StopIteration:
                    yield text
                    return StopIteration
                while is_joinpad(comp_next):
                    if isinstance(comp_next, MdAtomic):
                        text += pad + comp_next.make()
                    else:
                        text += pad + comp_next
                    try:
                        comp_next = next(citer)
    ...
    if isinstance(comp_prev, MdAtomic):
        yield comp_prev.make()
    elif comp_prev not in (' ', '\n'):
        yield comp_prev

  
def render_sub(components, span=np.inf):
    cur_width = 0
    text = ''
    for ntext in components:
        if ntext.count('\n'):
            cur_width = 0
            text += ntext
        else:
...
class MdCell(MdBase):
    ...
    def __init__(self, components):
        ...
        self.components = list(cell_strings(comps))
    def make(self, span=np.inf):
        return render_sub(self.components, span)
    @property
    def min_span(self):
        return cell_min_span(self.components, self._min_span)
    @property
    def begline(self):
        return 0
    @property
...
trail_newline_regex = re.compile(r'(.*?)(\n*)$', re.DOTALL)
trail_newline = lambda text: len(trail_newline_regex.match(text).group(2))

trail_whitespace_regex = re.compile(r'(.*?)(\s*)$', re.DOTALL)
trail_whitespace = lambda text: len(trail_whitespace_regex.match(text).group(2))


def min_whitespace_pad(ptext, ntext, bpad, epad, wspace, pad):
    trail = wspace(ptext)
    ntext = pad * (bpad - trail) + ntext
    if pad != ' ':
        trail = wspace(ntext)
    else:
        trail = epad
    return ptext + ntext + pad * (epad - trail)
...
def render_RST(elements):
    ...
    try:
        ...
        return ''
    if elem.endline:
        text = elem.make() + elem.pad_space * elem.endline
    else:
        text = elem.make() + elem.pad_space
    for elem in elements:
        if elem.endline or elem.begline:
            text = min_whitespace_pad(text, elem.make(), elem.begline, elem.endline, trail_newline, elem.pad_space)
        else:
            text = min_whitespace_pad(text, elem.make(), 1, 1, trail_whitespace, elem.pad_space)
...
if __name__ == "__main__":
    ...
    md_dir = Path('source/notebooks')
    # convert all notebooks
    MdBase.base_url = 'https://pyFCS.readthedocs.io/en/latest/'
    ...
    for file in iter_dir(MdBase.nb_dir):
        if file.parts[-1][-6:] != '.ipynb':
            continue
        convert_notebook(file, nb_dir, source_dir, md_dir)
    MdBase.copy_files(source_dir)
